...
import static com.yxf.log.Utils.checkNotNull;
import static com.yxf.log.YxfLog.ASSERT;
import static com.yxf.log.YxfLog.DEBUG;
import static com.yxf.log.YxfLog.ERROR;
import static com.yxf.log.YxfLog.INFO;
import static com.yxf.log.YxfLog.VERBOSE;
import static com.yxf.log.YxfLog.WARN;
...
public class YxfLog {
    ...
    private static int defaultTAGLength = 30;
    ...
    private static void update() {
        int length = MAIN_TAG.length();
        if (length < defaultTAGLength) {
            realTAG = MAIN_TAG + getBlank(defaultTAGLength - length);
        } else if (length == defaultTAGLength) {
            realTAG = MAIN_TAG;
        } else {
            realTAG = MAIN_TAG.substring(defaultTAGLength);
        }
    ...
    public static class SubLog {
        ...
        private SubLog(Builder builder) {
            ...
            if (builder.SUB_TAG == null || builder.SUB_TAG.length() < 1) {
                ...
                TAG = YxfLog.MAIN_TAG + "." + builder.SUB_TAG;
            }
            int length = TAG.length();
            if (length < builder.TAGLength) {
                realTAG = TAG + getBlank(builder.TAGLength - length);
            } else if (length == builder.TAGLength) {
                realTAG = TAG;
            } else {
                realTAG = TAG.substring(builder.TAGLength);
            }
            PrettyFormatStrategy strategy = PrettyFormatStrategy.newBuilder()
